...
export default async function AuthorPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  // 1. Декодируем имя из URL
  const authorName = decodeURIComponent(name);

  // ДИАГНОСТИКА
  console.log('--- AUTHOR DEBUG ---');
  console.log('Searching DB for Author:', authorName);

  // 2. Запрос постов 
  // ВАЖНО: Добавил 'excerpt' в выборку, чтобы было как на главной
          ...
          {posts?.map((post) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Ярлык автора (сохраняем стилистику сетки) */}
                  {post.author && (
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  {/* МЕТА-ДАННЫЕ (Теперь СВЕРХУ, как в сетке) */}
                  ...
                  {/* EXCERPT (Третьим, из поля excerpt) */}
                  {hasExcerpt && (
