...
export default function HeroSlider({ posts }: { posts: Post[] }) {
  ...
  const titleLen = activePost.title.length;
  
  // === НАСТРОЙКА РАЗМЕРА ШРИФТА ЗАГОЛОВКА ===
  let titleClass = "text-4xl md:text-6xl lg:text-7xl uppercase"; 
  let lineClampDesc = "line-clamp-[6]";      

  if (titleLen > 60) {
      titleClass = "text-2xl md:text-4xl lg:text-5xl uppercase";
      lineClampDesc = "line-clamp-[4]"; 
  } else if (titleLen > 30) {
      titleClass = "text-3xl md:text-5xl lg:text-6xl uppercase";
      lineClampDesc = "line-clamp-[5]";
  }

  const displayTitle = activePost.title 
    ? activePost.title.charAt(0).toUpperCase() + activePost.title.slice(1) 
    : '';
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div className="relative w-full max-w-[1500px] h-full flex items-center justify-center">
            ...
            {/* --- LEFT PREVIEW (BEHIND) --- */}
            <div 
            className="absolute left-0 md:left-[2%] w-[20%] md:w-[25%] h-[280px] md:h-[380px] z-10 
            ...
            <div 
            className="absolute right-0 md:right-[2%] w-[20%] md:w-[25%] h-[280px] md:h-[380px] z-10 
                        transition-all duration-700 ease-in-out
                        pointer-events-none hidden md:block"
            >
            ...
            <div className="relative z-20 w-full md:w-[70%] max-w-[1000px] h-full md:h-[450px] 
                ...
                {/* 
                    BUTTONS UPDATED:
                    Added drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]
                    This creates a hard shadow around the SVG path itself, ensuring visibility on light backgrounds without performance cost.
                */}
                <button 
                    onClick={(e) => {e.stopPropagation(); handlePrev()}} 
                    className="absolute -left-16 top-1/2 -translate-y-1/2 hidden md:flex items-center justify-center 
                            w-12 h-12 text-white transition-all duration-300
                            z-30 hover:scale-110 opacity-70 hover:opacity-100
                ...
                <button 
                    ...
                    className="absolute -right-16 top-1/2 -translate-y-1/2 hidden md:flex items-center justify-center 
                            w-12 h-12 text-white transition-all duration-300
                            z-30 hover:scale-110 opacity-70 hover:opacity-100
                            drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]"
                ...
                <div className="flex justify-between items-center border-b border-neutral-900 pb-4 mb-6 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        {activePost.author ? (
                            <Link 
                            href={`/author/${activePost.author}`}
                            className="text-sm font-mono font-bold text-neutral-400 uppercase tracking-widest hover:text-white transition-colors z-30"
                            >
                            {activePost.author}
                            </Link>
                        ) : (
                            <span className="text-sm font-mono font-bold text-neutral-400 uppercase tracking-widest">
                            UNKNOWN
                            </span>
                        )}
                    ...
                    <span className="text-sm font-mono text-neutral-500 tracking-widest">
                        {dateTech}
                    </span>
                </div>

                {/* 2. MAIN CONTENT */}
                <div className="flex-grow flex flex-col justify-center min-h-0 relative">
                    
                    <Link href={`/post/${activePost.id}`} className="block w-full">
                        <div className={`flex items-start ${!hasExcerpt ? 'mb-0' : 'mb-1'}`}>
                            <h2 className={`
                                font-serif text-[#f0f0f0] leading-[0.9] tracking-tight hover:text-white transition-colors line-clamp-3
                                ${titleClass}
                            `}>
                                {displayTitle}
                            </h2>
                    ...
                    {hasExcerpt && (
                        <p className={`font-serif text-neutral-300 text-lg md:text-xl leading-relaxed ${lineClampDesc}`}>
                        {activePost.excerpt}
                        </p>
                    )}
                </div>
                ...
                <div className="mt-auto pt-4 border-t border-transparent shrink-0 flex justify-end items-center">
                    <Link 
                        ...
                        className="group flex items-center gap-3 px-6 py-3 border border-neutral-800 hover:bg-neutral-900 transition-all"
                    >
